...
你好，我是江小湖。在 [记忆三层模型](./01-memory-layers.md) 中，你了解了短期、工作、长期三层记忆的分工。但长期记忆有一个关键问题没有展开：**记忆存在哪里？怎么在需要的时候准确地取出来？** 这篇文章解决核心问题：**存储后端的选型、向量检索的原理、以及如何让记忆检索的准确率从"能用"提升到"可靠"**。
