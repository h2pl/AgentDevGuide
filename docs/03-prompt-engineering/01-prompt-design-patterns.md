...
**动态 Few-shot**：在 Agent 开发中，示例不应该写死在 Prompt 里。更好的做法是维护一个示例库，根据用户输入动态检索最相关的示例拼接到 Prompt 中。这是 RAG 和 Prompt 工程的一个重要结合点，我们会在第 06 章详细展开。
...
**CoT 对 Agent 开发的意义**：CoT 是 Agent **任务规划**的技术基础。在 ReAct 模式中（第 05 章会详细展开），Agent 在每一步行动前都会"想一想"当前状态和下一步行动——这个"想"本质上就是 CoT 的应用。有了 CoT，Agent 才能将复杂目标拆解为可执行的子任务序列。
...
> 掌握了"让模型说什么"，下一步是"让模型怎么说"——结构化输出是 Agent 与工具交互的桥梁。接下来请阅读 [结构化输出：让 LLM 输出稳定的 JSON](./02-structured-output.md)。
