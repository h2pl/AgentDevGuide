...
| # | 文章 | 内容 |
|---|------|------|
| 01 | [认识大语言模型：LLM 的定义、原理与生态全景](./01-llm-overview.md) | LLM 的定义、核心原理、主流模型 |
...
| 05 | [LLM 的局限与工程对策：RAG、工具调用、记忆管理如何补齐](./05-limitations.md) | 每个局限的架构根因、具体失效案例、Agent 工程对策 |
| 06 | [Token 与 Embedding：文本是如何变成向量的](./06-token-and-embedding.md) | 分词原理、语义向量、Token 对成本和上下文的影响 |
| 07 | [Transformer 内部原理：自注意力、因果掩码与 KV Cache](./07-transformer-internals.md) | Block 结构、注意力计算、多头注意力、因果掩码、KV Cache |
