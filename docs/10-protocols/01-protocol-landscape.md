...
你好，我是江小湖。从第 04 章的工具调用到第 09 章的框架编排，你的 Agent 已经具备了完整的执行能力。但这里有一个隐性问题：所有工具和交互都绑定在特定的模型和实现上——换个客户端就得重写集成，换个厂商就得重新适配。
...
实现细节在 [第 04 章「MCP 与工具生态」](../04-tool-use/04-mcp-and-tool-ecosystem.md) 中已有详细讲解，本章不再重复。第 02 篇会从协议治理和企业部署的角度做深入分析。
