...
同一架构在训练的不同阶段会产出不同变体。这是 HuggingFace 上下载开源模型时**最容易踩坑**的地方。以下只做能力对比，**训练流程的具体机制和目标函数差异**见 [从 Base 到 Chat](../01-llm-basics/08-training-pipeline.md#变体如何从训练流程中产生)。
...
部署形态变体主要解决**"我的 GPU 跑不动原版模型"**或**"我想要更小的模型"**的问题。以下只做选型对比，**量化/蒸馏/合并/剪枝的技术原理和区分**见 [从 Base 到 Chat](../01-llm-basics/08-training-pipeline.md#后处理变体不是训练出来的是加工出来的)。
