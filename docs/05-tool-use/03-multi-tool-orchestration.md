...
**为什么上下文膨胀还影响准确率**：自注意力机制对上下文中的所有 Token 计算关联度。上下文越长，注意力权重越分散——与当前决策相关的关键信号（最新的用户请求和工具结果）被大量历史 Token 稀释。这就是"迷失在中间"现象（[LLM 的局限与工程对策](../02-llm-basics/05-limitations.md)中讲过）在多工具场景中的具体表现。
